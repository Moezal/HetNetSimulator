 ...
 * @author mohamad
 */
public class GlobalOptimizationHandoverFullProfit {
            ...
            for (int i = 0; i < peersServiceLevel.size(); i++) {
                for (int j = 0; j < Networks.size(); j++) {
                    if (peersServiceLevel.get(i).isNetworkAvailable(Networks.get(j))) {
                        globalCostFunction[i][j] = NetworkCostFunctionSmaller.calculate_Network_cost_function(peersServiceLevel.get(i), Networks.get(j), globalMaxRss, globalMinPC) / peersServiceLevel.get(i).getDatarate();
