 ...
 * @author mohamad
 */
public class profitFunctionBased {
...
//            double[][] globalCostFunction = new double[peersServiceLevel.size()][Networks.size()];
            ...
            for (int i = 0; i < peersServiceLevel.size(); i++) {
                List<ProphitWeightRatio> pwr1Peer = new LinkedList<ProphitWeightRatio>();
                for (int j = 0; j < Networks.size(); j++) {
                    if (peersServiceLevel.get(i).isNetworkAvailable(Networks.get(j))) {
                        ProphitWeightRatio Pnow = new ProphitWeightRatio(peersServiceLevel.get(i).getNetworkRank(Networks.get(j)), peersServiceLevel.get(i));
...
//                System.out.println(""+pwrNow.getRatio());
                if (!pwrPeer.isConnected()) {
//                    System.out.println(""+pwrPeer.getPeerID());
