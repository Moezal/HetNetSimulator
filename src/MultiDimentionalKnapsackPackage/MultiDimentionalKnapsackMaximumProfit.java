package MultiDimentionalKnapsackPackage;

public class MultiDimentionalKnapsackMaximumProfit {
    ...
    private static void noRep(Item[] items) {
        ...
        for (int i = 1; i <= items.length; i++) {
            for (int j = 0; j <= size; j++) {
                for (int h = 0; h <= vol; h++) {
                    if (items[i - 1].size > j) // If the item i is too big, I  can't put it and the solution is the same of the problem with i - 1 items
                    {
                        dynNoRep[i][j][h] = dynNoRep[i - 1][j][h];
                    } else {
                        if (items[i - 1].volume > h) // If the item i is too voluminous, I can't put it and the solution is the same of the problem with i - 1 items
                        {
                            dynNoRep[i][j][h] = dynNoRep[i - 1][j][h];
                        } else {
    ...
    public static void main(String[] args) {
        size = 40;
        vol = 5;
        Item[] items = {new Item(370, 8, 1), new Item(370, 8, 1), new Item(370, 8, 1), new Item(370, 8, 1), new Item(370, 8, 1),
            new Item(3820, 32, 1), new Item(3820, 32, 1), new Item(3820, 32, 1), new Item(382, 32, 1), new Item(382, 32, 1),
            new Item(414, 16, 1), new Item(414, 16, 1), new Item(414, 16, 1), new Item(414, 16, 1), new Item(414, 16, 1)};
