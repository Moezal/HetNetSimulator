...
//        }
        ...
        LinearProgram lp = new LinearProgram(newEquationArray);
...
//            }
//            System.out.println(" }");
            lp.addConstraint(new LinearBiggerThanEqualsConstraint(newConstraintsTranspose.get(i), equationArrayWithoutZerosArray[i], "constraint" + i));
//            lp.addConstraint(new LinearBiggerThanEqualsConstraint(d, 0.0, "constraint" + constraintCounter++));
        }
        double[] zeroArray = new double[newEquationArray.length];
        for (int i = 0; i < zeroArray.length; i++) {
            zeroArray[i] = 0;
        }
        double[] oneArray = new double[newEquationArray.length];
        for (int i = 0; i < zeroArray.length; i++) {
            oneArray[i] = 50;
        }
//        for (int i = 0; i < newEquationArray.length; i++) {
//            lp.setContinous(i);
//        }
        lp.setLowerbound(zeroArray);
        lp.setUpperbound(oneArray);
