 ...
 * @author mohamad
 */
public class BranchAndBoundNetworkProcessingThread  {

    Network n;
//    public boolean isAlive = true;

    public BranchAndBoundNetworkProcessingThread(Network n) {
        this.n = n;
...
//                    }
        ...
        if (n.getFreeCapacity() >= n.getTotalRequestedDataRate()) {
            ...
            n.setPeersRequestingConnection(new LinkedList<Peer>());
            double[] costFunctionArray = new double[peersToBeProcessed.size()];
            double[] usersDataRateArray = new double[peersToBeProcessed.size()];
            int loopCounter = 0;
            for (Peer p1 : peersToBeProcessed) {
                usersDataRateArray[loopCounter] = p1.getDatarate();
                costFunctionArray[loopCounter] = NetworkCostFunction.calculate_Network_cost_function(p1, n, max_mc, max_pc, max_s, min_mc, min_pc, min_s, overall_requested_BUs, overall_mc, overall_pc, overall_s);
                loopCounter++;
            }

            double[] knapsackSolution = KnapsackOptimizationSolver.KnapsackOptimizationParser(costFunctionArray, usersDataRateArray, n.getCapacity());
            List<Integer> peersAccepted = new LinkedList<Integer>();
            for (int f = 0; f < knapsackSolution.length; f++) {

                if (knapsackSolution[f] == 1) {
                    peersAccepted.add(peersToBeProcessed.get(f).getPeerID());
                }
            }
